<template lang="html" src="../../views/pagamento/list.html"></template>
...
export default {
	...
	mounted() {
		...
		this.root = this.$root.$refs.payment
	...
	methods: {
			...
			this.dataTable = $(this.table).DataTable({
				...
				drawCallback: function() {
					...
					$(".select").off("click").on("click", function( event ) {
						...
						if( id == 1 ) {
							self.form.type = "pagseguro" 
						}

						if( id == 4 ) {
							self.form.type = "paypal" 
						}
		...
		GetRowsDataTable() {
			...
			src.query().then(function( res ) {
				...
				this.form.pagseguro = model.pagseguro[0]

				this.form.pagseguro = Object.assign({}, this.form.pagseguro, {"nome":data[0]["nome"]})
