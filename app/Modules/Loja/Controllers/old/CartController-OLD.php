	...
	public function buildPayment(Request $request)
	{
		...
		for($i = 0; $i < count($cart->imagens); $i++){
			$itens[$i] = [
				...
				'itemAmount' => $cart->precoUnidadeImagem,
				'itemQuantity' => '1'
			];
		}
		...
		for($j = 0; $j < count($cart->politicas); $j++){
			$itens[$contador] = [
				...
				'itemAmount' => $cart->politicas[$j]['preco'],
				'itemQuantity' => '1'
			];
			$contador++;
		}
		...
		try {
			...
			$pagseguro = PagSeguro::setReference($idPedido)
				...
				->setShippingInfo([
					...
					'shippingCost' => $cart->frete['valor']
				])
				->send([
					'paymentMethod' => 'creditCard',
					'creditCardToken' => $request->input('token'),
					'installmentQuantity' => ($request->input('parcelas') > 0) ? $request->input('parcelas') : 1,
					'installmentValue' => ($request->input('valor_parcela') == 0.00) ? $request->input('valorTotal') : $request->input('valor_parcela'),
		...
		$this->buildOrder($idPedido, $retornoPagSeguro, $request, $itens);
