	...
	<section id="finaliza-compra">
		...
		<div class="container">
						...
						<div class="body">
							<div class="col-sm-12">
								...
								<div class="row">
									...
									<div class="col-sm-3">
										<?php $total = number_format(($cart->precoTotal + $cart->frete['valor']) - $cart->valDesconto, 2,",",".") ?>
										<b>R$ <?php echo $total < 0 ? '0,00' : $total; ?></b>
									</div>
								</div>
			...
			<div class="col-md-6">
				<br>
				<div class="clearfix"></div>
				<a id="finalizaCompraGratis" href="#" class="btn btn-blue pull-right next-button">Finalizar Compra</a>
			</div>
		</div>
...
@section('scripts')
	...
	<script>
		...
		$(document).ready(function(){
			...
			$('#finalizaCompraGratis').off('click').on('click', function(e){
				e.preventDefault();

				if( $('input[name="name"]').val().length < 2 ){					
					swal({
						title: "Erro com o Nome",
						text: "Por favor, corrija para prosseguir!",
						type: 'error',
						closeOnConfirm: true,
					});
				...
				if( $('input[name="data_nascimento"]').val().length < 10 ){					
					swal({
				...
				if( $('input[name="celular"]').val().length == 0 ){		
					swal({
						title: "Erro com o número de Celular",
						text: "Por favor, corrija para prosseguir!",
						type: 'error',
						closeOnConfirm: true,
					});
				...
				$.ajax({
					url: '{{ url("/realiza-pagamento-gratis") }}',
					method: 'POST',
					...
					error: function (xhr, ajaxOptions, thrownError) {
						...
						swal({
							...
							text: "Por favor confira todos os dados e tente novamente!",
							timer: 5000,
							showConfirmButton: false,
							type: 'error'
						});
