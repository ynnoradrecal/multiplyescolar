	...
	<header>
		...
		<?php /* @if( Request::session()->get('has_cart') && Request::session()->get('session_id') !== Request::session()->get('last_session_id') && Request::session()->get('last_session_id') !== null && Request::url() != url('/home') && Request::url() != url('/'))
			
			<div id="recupera-carrinho" class="">
				<div class="container">
					<div class="col-md-8 pull-right text-right">
						<span>Deseja voltar para sua última compra? 
						<a href="{{ url('/add-to-cart/images')}}" class="btn btn-success btn-md">Retornar a última compra</a></span>
					</div>
				</div>
			</div>
		...
		@if( Request::session()->get('ccart') != 0 )
			<div id="recupera-carrinho" class="">
				<div class="container">
					<div class="col-xs-12 col-md-12 pull-right text-center">
						<a href="{{ url('')}}/minha-conta/cart-abandonado" class="btn btn-success btn-md">
							<span class="glyphicon glyphicon-shopping-cart" aria-hidden="true"></span>
							Você Possui Histórico de Carrinho não Concluido!
						</a>
					</div>
				</div>
			</div>
		...
		<div class="container-fluid  <?php echo $class; ?> navbar ">
							...
							/* <li id="top-cart">
										...
										@if (Session::has('cart'))
											...
											@if( count($cart->politicas) > 0 )
												...
												<li class="politica text-left">
													...
													<?php foreach($cart->politicas as $item): ?>
														<?php $item = json_decode(json_encode($item), false); ?>
														<?php $titulo .= $item->titulo. '<br>'; ?>
														<?php $politicasSubTot += $item->preco; ?>	
													<?php endforeach; ?>

													<? /* php for( $i = 0; $i < count($cart->politicas); $i++): ?>
											...
											@if ( $cart->valDesconto > 0 )
												<li class="descontos" style="background: #333;">
													<span style="font-size: 12px;">
														Descontos: 
														R$ {{ number_format($cart->valDesconto, 2,",",".") }} 
													</span>
												</li>
											@endif
								...
								{{--  <div class="sub-menu">
									...
									<ul class="">
										...
										@if( Request::session()->get('session_id') !== Request::session()->get('last_session_id') && Request::session()->get('last_session_id') !== null && Request::url() != url('/home') && Request::url() != url('/'))
											<li><span style="background: #5AB75D !important;"><a href="{{ url('/add-to-cart/images')}}">Retornar a <br>última compra</a></span></li>
										@endif										
